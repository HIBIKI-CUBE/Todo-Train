...
任意の `nextBlockTitle` / `nextBlockStartsAt` / `nextBlockEndsAt` / `timetablePauseAt` は乗務中のいま、なければ次の占有（ダイヤまたは掲示）。欠けていても旧クライアントは無視する。Encode は値があるときだけ出す。時刻と分は Date からクライアントが床する。残り秒は送らない。一行だけ。Hub のいま＋次の二行は iPhone 側。
