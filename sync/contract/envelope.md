...
任意の `nextBlockTitle` / `nextBlockStartsAt` / `nextBlockEndsAt` / `timetablePauseAt` は乗務中の次（またはいま重なっている）ダイヤ。欠けていても旧クライアントは無視する。Encode は値があるときだけ出す。分は Date からクライアントが床する。残り秒は送らない。
