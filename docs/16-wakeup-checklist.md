...
最終更新: 2026-09-13。コード側で進められる引き継ぎは PR にした。ここは **人が見ないと閉じない** ものだけ。
...
- [ ] iPhone で発車・停車・到着しても、Mac は「つなぎ直す」なしで追いつく（WS。切れても hint で数秒以内）
...
- [ ] 乗務中 PiP が四隅へスナップする。左右の外へ十分ドラッグすると同じカードのまま画面の外へ出る。端は色付きの縦タイトル、ホバーでシェブロン。クリックで戻る。隣のディスプレイへは移る（追い出さない）
...
- [ ] ログイン時起動が既定 ON
- [ ] 30 分切符: Mac PiP に「まだ乗ってる？」が 1–2 回。端に出していても戻って出る
- [ ] 片方でまだやってる（または停車）ともう片方は同じ回を出さない
- [ ] Mac 設定の車内放送 OFF で PiP は沈黙。iPhone OFF でも Mac は出さない
- [ ] ペア済みで iPhone を背面にしても進捗のローカル通知は出ない（away LA と Focus 4 択は残る）
- [ ] 運行は開いて乗っていない: 12–20 分で「運行は続いてる」（まだやってるだけ）。Mac も同じ通知。PiP は出ない。発車したら消える
- [ ] ペア済みでも iPhone の idle 通知は残る。片方でまだやってるともう片方は終わる。Hub を眺めただけでは延びない
