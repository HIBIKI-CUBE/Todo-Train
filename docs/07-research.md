...
状況認識（次枠・いま被っている枠）は案内板と Focus / PiP、Hub 運行の一行に出す。Hub に掲示リストは常設しない。許可済みなら Hub でも掲示を温める（表示はしない）。発車が見積と載せた枠、またはいまの掲示と重なるときは確認する。発車は止めない。日常との摩擦は [17](17-calendar-experience.md)。
