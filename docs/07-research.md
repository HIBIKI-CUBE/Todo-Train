...
状況認識（次枠・いま被っている枠）は案内板と Focus / PiP、Hub 運行の一行に出す。題名と分。Hub に掲示リストは常設しない。許可済みなら Hub でも掲示を温める（表示はしない）。発車・停車・再乗車は確認しない。載せた枠と重なれば ATS が網になる。載せる／外すは案内板と図表。未載せの掲示に重なっているあいだは away を出さない（責めにしない。away を網にはしない）。日常との摩擦は [17](17-calendar-experience.md)。
