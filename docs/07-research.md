...
状況認識（いま被っている枠と次の枠）は案内板と Focus / PiP、Hub 運行のいま＋次に出す。案内板は 60 分の 5 分セル（近い枠では伸びる）。Hub peek の同じ尺に占有の門。占有は行先票（題名が主、時刻と分は桁）。Focus と PiP は乗務の進捗バー。未載せも含む。Hub に掲示リストは常設しない。許可済みなら Hub でも掲示を温める。発車・停車・再乗車は確認しない。載せた枠と重なれば ATS が網になる。ATS の停車は上限を食わない。枠が終わると静かな案内は消える。載せる／外すは案内板と図表。未載せの掲示に重なっているあいだは away を出さない（責めにしない。away を網にはしない）。日常との摩擦は [17](17-calendar-experience.md)。
