...
| 設計 | 根拠 | 強度 |
|------|------|------|
| 停車上限 2（設定で 3） | [Koechlin et al. 2010](https://doi.org/10.1126/science.1183614) 同時ゴール ≈2; [Leroy 2009](https://doi.org/10.1016/j.obhdp.2009.04.002) 注意残留; [Cowan 2001](https://doi.org/10.1017/S0140525X01003922) WM 3–4; Kanban 個人 WIP | **強〜中** |
...
| 定時は帯域内・非通貨 | Goodhart: 測ると歪む。早着祝いは見積もり水増しを合理化する。[Deci & Ryan SDT](https://doi.org/10.1037/0003-066X.55.1.68) 内発 > 点数 | **中** |
...
警告のみは弱い。**ソフトブロック + 解決選択**（停車中を片付ける / 途中下車 / 放棄 / 臨時停車）。
...
| 定時は同じ案内の見出し味付け。見積/実績は定時のときだけ | 早着・超過の差分を祝祭で突き付ける |
| 当初見積もりで定時判定。延長後予算では見ない | 延長して「間に合わせた」ことにする |
| 消える瞬間 + haptic。履歴は「定時」or「到着」 | ストリーク / XP / 定時率 / 解除 / 「延着」バッジ |
| 空の運行・到着ゼロは定時運行にしない | 運行開始→即終了で祝う |

定時だけ祝うと見積もり水増しの動機になる。完了を先に祝うと、その動機も薄くなる。
...
```
1 Live Activity = 1 乗務（発車中）
≠ 1 運行日
```

- 停車中切符は LA に並べない  
- LA 削除 ≠ 運行キャンセル（DB が真実源）  
- 停車中は `timerInterval` を止められない → 静的残り時間表示に切替
