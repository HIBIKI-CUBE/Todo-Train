...
| やる | やらない（ハックされる） |
|------|--------------------------|
| 当初見積もりに近い到着だけ短い案内 | 早着も祝う（見積もり水増し） |
| 延長後予算ではなく当初ダイヤで判定 | 延長して「間に合わせた」ことにする |
| 消える瞬間 + haptic。履歴は「定時」バッジのみ | ストリーク / XP / 定時率 / 解除 |
| 空の運行・到着ゼロは定時運行にしない | 運行開始→即終了で祝う |

待って帯域に入れることは原理上できる。祝祭に価値（点数）を載せないので、待つ理由が薄い。
