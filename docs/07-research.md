...
状況認識は乗務の現場に出す（Lock / LA / Widget には出さない）。掲示も含む。Hub に掲示リストは常設しない。ATS の停車は上限を食わない。枠が終わると静かな案内は消える。掲示のまま重なっているあいだは away を出さない（away を網にはしない）。見た目は [12](12-ui-design.md)。日常との摩擦は [17](17-calendar-experience.md)。
