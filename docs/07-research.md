...
カレンダーは参照。所属（載せる／外す）はアプリ内が正。EventKit の ID をライブの所属にしない。時刻の変更・取消は読み直してよい。

**車内放送 away はロック中は出ない。** 会議は Mac、iPhone は伏せてあるのが普通なので、away を待避の網にしない。

通知と AlarmKit はバックグラウンドの SwiftData を任意時刻に書き換えられない。「その瞬間に OS が停車した」とは見せない。開始時は停車通知。60 秒未応答で保護境界を保存し、前面ならその場、背景なら次の `reconcile()` で境界へ遡って走行を切る。Mac が接続していれば PiP の時計が同じ境界で停車コマンドを送る。

AlarmKit は終了ベルとダイヤ開始を 2 本にしない。`fireAt` は予算終了と次のダイヤ開始の早い方。見積到達は超過して走ってよい。ダイヤの保護境界だけが停車させる。
