...
定時の喜びは **エフェメラな駅案内**（`Punctuality`）。ストリークや定時率は出さない。
...
| 定時到着 / 定時運行 | ✅ 帯域内のみ短い案内。履歴は「定時」バッジ。率・ストリークなし |
...
2. 発車 → Focus（停車 / 到着 / 延長 / 超過）。定時なら Focus 閉鎖後に短い案内
3. 停車上限 → 解決シート / 途中下車キャンバス / 臨時停車
4. 履歴で振り返り・乗り継ぎリンク・今日に追加（定時はバッジのみ）
5. 運行終了 → 停車中の持ち越し禁止 / 途中下車 / 放棄。到着がすべて定時なら短い「定時運行」
