...
| Mac PiP | iOS のピクチャ・イン・ピクチャに近い黒カード。経過バーは Focus と同じ `FocusTimerPhase` の段階色。残り数字はバーの進捗によらず常に読む（塗りと未塗りの境でも対比を保つ）。Focus コックピットは複製しない。詳細は [14](14-mac-companion-ux.md) |
...
3. **車内放送**: 超過と同じ操作盤差し替え。予告カウントなし。背面（他アプリ）は Session LA の alert。ロック中は出さない。
